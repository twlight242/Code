...
void calculateCscan(int requests[], int n, int initial_head, int disk_size, int direction){
	...
	if (direction == 1)
	{
		...
		if(currentHead < disk_size - 1){
			int end_seek_time = abs(disk_size - 1 - currentHead);
		...
		int reset_seek_time = disk_size - 1;
		...
		if(currentHead > 0){
			int start_seek_time = abs(currentHead - 0);
			totalseektime += start_seek_time;
			printf("| _ | 0 | %3d | \n", start_seek_time);
		}
...
int main(int argc, char const *argv[])
{
	...
	calculateCscan(requests, n, head, disk_size, direction);
